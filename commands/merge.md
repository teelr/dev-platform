...
**Every `#<number>` in this report carries a type word — `PR #384`, `issue #383` — and an issue in another repo is cited as `<owner>/<repo>#<n>`, the exempt form.** A bare cross-repo number is the worst case: it names an object in a repo the reader is not looking at, in a number space shared with PRs. See "Which Identifier To Cite" in `CLAUDE.md`.
...
2. **Generate the Change Summary — runs every time, unconditionally, before anything else below.** Produce a short **Problem/Opportunity → What shipped** summary using this fallback order (stop at the first tier that yields content):
   - **Tier 1 (preferred):** the shipped file this merge added. Run `git diff HEAD~1 --name-only -- tasks/shipped/`. Take this tier **only when that returns exactly one file** — then use its content directly, lightly trimmed if needed, not rewritten. This is the narrative `/code` already wrote pre-commit, already reviewed by `/review`. **Two or more files means this merge backfilled the directory rather than shipping a phase** — a project adopting `tasks/shipped/` migrates its whole history in one merge, and not one of those files describes the change being merged; fall through to Tier 2. Picking the first of them instead reports an old phase as today's work: on `teelr/kermit-harness#400` the 14 migrated files would have summarised a docs migration merged 2026-09-07 as `v4.24.0`, shipped 2026-05-23 (`teelr/dev-platform#115`). **Legacy fallback** (projects without `tasks/shipped/`): `git diff HEAD~1 HEAD -- planning.md` — if it shows added lines under a "Recently shipped" (or equivalently-named changelog) section, use that prose the same way; the same caution applies, since the migration that retires that section also touches it.
   - **Tier 2:** If Tier 1 was skipped or yielded nothing (no shipped file added, more than one added, or no legacy `planning.md` changelog lines) but Step 1 found a spec, use the spec's Problem statement / Design Philosophy opening + its Change titles from the Overview section.
...
5. **Detect Roadmap-Phase completion**, whether or not the spec had its own Post-merge section. Determine whether this merge shipped the *last Change of a Roadmap Phase*:
   ...
   - If complete, **execute the standard Roadmap-Phase-completion actions**: mark the phase complete in `ROADMAP.md` + `planning.md` (today's date + status), close the GitHub milestone (`gh api -X PATCH repos/:owner/:repo/milestones/<n> -f state=closed`, or `./scripts/sync-milestones.sh --apply` where the project ships it), and **cut the release tag** at this merge's squash commit, named exactly as the phase version:
     ...
     **The `<n>` in the `gh api` call stays a number — that is the correct API argument. The REPORT does not.** Say `milestone v<X.Y>: <Title> closed`, never `milestone #<n> closed`: the milestone number is a sequential counter unrelated to the version it holds, so it tells the reader nothing. You already have the title — it is the milestone you just closed.
...
6. **Land any file changes from steps 4-5.** If the project's rules forbid direct commits to `main` (check its CLAUDE.md), run the SAME mini-cycle this skill already knows how to do — reuse Steps 1-5 above on a fresh branch:
   - **Cut the branch worktree-aware, same as feature work.** Check `test -f .claude/worktree-deps` from the current directory (by this point in Step 7 the session is always back in the project's main checkout — Step 5 already returned it there, whether the feature branch itself used branch mode or worktree mode):
     - **Worktree-opted-in** (`.claude/worktree-deps` present): don't `git checkout -b` in the shared main checkout — that's the exact contamination this Change exists to prevent. Instead, mirror `/code`'s worktree-mode branch creation: call the **`EnterWorktree`** tool with `name` set to `chore/<slug>`, then link the project's heavy git-ignored deps into it with `bash ~/.claude/worktree/link-deps.sh` — exactly as written, no arguments (the script derives both paths; a worktree-isolated session's command guard refuses any invocation containing a variable or substitution). The session is now re-rooted into `.claude/worktrees/chore/<slug>`.
...
7. **Report what post-merge did** — starting with the Change Summary block from sub-step 2, then the actions taken (files changed, milestone closed by title, doc-update PR's own merge commit SHA if sub-step 6 ran), or "nothing further to do" if sub-steps 4-5 found nothing. Identifiers follow Step 6's rule: type word in front of every `#<number>`, milestones by title, `<owner>/<repo>#<n>` for anything in another repo.
