...
Cutting the tag is a mechanical post-merge sub-step, not a thing to remember — see the post-merge bullet below. It previously lived only in `docs/GLOSSARY.md`, a reference doc nobody executes from, and consequently stopped after v1.13 while every consumer stayed pinned to `@v0.7`. `scripts/check-phase-tags.sh` is the backstop.
