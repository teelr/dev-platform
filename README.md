...
See [ROADMAP.md](ROADMAP.md) for the spec sequence. Roadmap Phase headers use semver (`v<MAJOR>.<MINOR>: <Title>`) and map 1:1 to GitHub Milestones. v0.1 (Foundation) shipped 2026-05-08, v0.2 (Global Claude + Hooks) shipped 2026-05-09, v0.3 (Project Scaffolding) + v0.4 (Testing) shipped 2026-05-10, v0.5 (Monitoring) + v0.6 (VSCode Coverage Server-Side) + v0.7 Phase 1 (taxonomy enforcement extended to roadmaps) shipped 2026-05-11. Every `/gate fast` against dev-platform runs `./scripts/gate_fast.sh` mechanically (66 checks, < 20s). Run `./scripts/report.sh` for a daily/weekly/all metrics report (gate pass rate, /code retry counts, /review count, tool-execution-time average); `./scripts/sync-vscode.sh` for VSCode extension capture/deploy/diff. **v0.7 Team Enablement in flight** — Phase 1 shipped; Phase 2 (GitHub Actions CI + reusable taxonomy-check workflow + consumer adoption guide at `docs/CI-INTEGRATION.md`) implemented on branch `v0.7/phase-2-github-actions`; Phases 3–4 (GitHub Pages + GLOSSARY, Milestones automation) queued. v0.7 release tag cut at Phase 4 completion.
