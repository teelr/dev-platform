...
See [ROADMAP.md](ROADMAP.md) for the spec sequence. Roadmap Phase headers use semver (`v<MAJOR>.<MINOR>: <Title>`) and map 1:1 to GitHub Milestones. **v0.1 – v0.7 all shipped 2026-05-08 – 2026-05-11** (v0.7 Team Enablement closes with the `v0.7` release tag, the live Pages docs site at [teelr.github.io/dev-platform](https://teelr.github.io/dev-platform/), GitHub Actions CI required on every PR, and `scripts/sync-milestones.sh` keeping Milestones in sync with `ROADMAP.md`). Every `/gate fast` against dev-platform runs `./scripts/gate_fast.sh` mechanically (88 checks, < 30s) — locally AND in CI on every PR. Run `./scripts/report.sh` for a daily/weekly/all metrics report; `./scripts/sync-vscode.sh` for VSCode extension capture/deploy/diff; `./scripts/sync-milestones.sh` for ROADMAP→Milestones drift detection; `./scripts/fleet-gate.sh` (v0.8) to sweep gates across every active project in `monitoring/projects.json`. **v0.8 Cross-project orchestration in flight** — Phase 1 (registry + fleet-gate parallel sweep with per-project timeouts + telemetry emit) implemented on branch `v0.8/phase-1-registry-fleet-gate`; Phases 2–4 (Dashboard, Drift Correction, Pin Tracking) queued. v0.8 release tag cuts at Phase 4 completion.
