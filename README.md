...
See [ROADMAP.md](ROADMAP.md) for the spec sequence. Roadmap Phase headers use semver (`v<MAJOR>.<MINOR>: <Title>`) and map 1:1 to GitHub Milestones. v0.1 (Foundation) shipped 2026-05-08, v0.2 (Global Claude + Hooks) shipped 2026-05-09, v0.3 (Project Scaffolding) + v0.4 (Testing) shipped 2026-05-10, v0.5 (Monitoring) + v0.6 (VSCode Coverage Server-Side) + v0.7 Phases 1, 2, 3 (taxonomy enforcement extended to roadmaps; GitHub Actions CI + reusable taxonomy-check workflow + consumer adoption guide; `gate-fast` required status check on `main`; GitHub Pages docs site at [teelr.github.io/dev-platform](https://teelr.github.io/dev-platform/) + alphabetized [Glossary](https://teelr.github.io/dev-platform/GLOSSARY)) shipped 2026-05-11. Every `/gate fast` against dev-platform runs `./scripts/gate_fast.sh` mechanically (78 checks, < 30s) — locally AND in CI on every PR. Run `./scripts/report.sh` for a daily/weekly/all metrics report (gate pass rate, /code retry counts, /review count, tool-execution-time average); `./scripts/sync-vscode.sh` for VSCode extension capture/deploy/diff; `./scripts/sync-milestones.sh` for ROADMAP→Milestones drift detection. **v0.7 Team Enablement in flight** — Phases 1–3 shipped; **Phase 4** (Milestones automation: `scripts/sync-milestones.sh` + `tests/milestone-sync/` mock-`gh` round-trip suite) implemented on branch `v0.7/phase-4-milestone-sync`. **Phase 4 closes v0.7** — post-merge cuts the v0.7 release tag, which finally validates the `@v0.7` pin in the consumer template waiting since Phase 2.
