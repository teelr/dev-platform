...
- **Invoke it as `~/.claude/worktree/link-deps.sh`, never `"${HOME}"/...`.** A worktree-isolated session's command guard cannot statically verify a path built from a variable, so the `${HOME}` form is refused outright ("too complex to verify that it stays inside the worktree") and the linking step silently never runs. Passing the main checkout as an argument is fine — that was never the problem.
