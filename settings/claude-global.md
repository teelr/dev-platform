...
`/code` writes a new `tasks/shipped/` file where that convention exists (`planning.md` on legacy projects that lack the directory) and updates ROADMAP.md, README.md, and tasks/lessons/ as its final step, staging them all. If commit time arrives with stale docs (e.g. `/code` was interrupted), run `/docs` to recover — exception, not the norm.
