...
**Watch for nested `.md` files with live content checks.** The detector's bare `*.md` default matches any `.md` file at any depth, not just root-level ones — bash's `[[ str == pattern ]]` lets `*` cross `/`. dev-platform's own gate doesn't use the bare default for exactly this reason: `tests/commands/frontmatter.sh` validates `commands/*.md` (slash-command definitions — effectively code, not documentation) directly against the live repo, so `scripts/gate_fast.sh` overrides `DOCS_ONLY_ALLOW_PATTERNS` to `("README.md" "CLAUDE.md" "ROADMAP.md" "planning.md" "docs/*" "tasks/*")` instead — naming dev-platform's actually-inert paths explicitly rather than trusting a wildcard. If your project validates any nested `.md` file's live content (not a fixture copy) the same way, do the same: name that path out of the allowlist rather than relying on a broad `*.md`.
...
**v1.27 (main-checkout derivation).** Five fleet scripts — `fleet_pins.py`, `fleet_dashboard.py`, `fleet-gate.sh`, `fleet-status.sh`, `fleet-install-template.sh` — each derived their root from their own file location and then joined the registry's relative `projects/<name>` paths onto it. `projects/` is gitignored, so it exists only in the main checkout: run from a worktree, all five pointed at paths that do not exist, and **none of them errored**. `fleet-pins.sh` reported `— not adopted` for all seven consumers, which reads exactly like a real answer. v1.25 had already fixed this identical bug for `install.sh` and `verify.sh` and swept those two together — but the fix stayed inline in those two files, so when the fleet scripts needed the same value it became a fifth and sixth copy rather than a shared one. Extracted to `scripts/lib/main_checkout.{sh,py}` with `tests/fleet-worktree/`, which asserts the resolution from a real worktree and not just from `main`.

Four copies deliberately stay inline, and the helper's header says which and why: `gate-lock.sh` and `check-concurrent-sessions.sh` run from the deployed `~/.claude/worktree/` path where `scripts/lib/` does not exist, and `install.sh`/`verify.sh` are bootstrap scripts that `tests/worktree-default` copies into a fixture worktree *by themselves* — pointing those two at the helper was tried in v1.27 and cost five test failures. "One shared helper" means one for everything that can share it, plus a written reason for each that cannot.
